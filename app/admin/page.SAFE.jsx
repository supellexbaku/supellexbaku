...
export default function AdminPage() {
  ...
  async function deleteVacancy(id) {
    await fetch(`${API}/admin/vacancy/${id}`, {
      method: "DELETE"
    });
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={() => setTab("products")} style={{
            ...(tab === "products" ? premiumButton : ghostButton),
            textAlign:"left"
          }}>
            📦 Məhsullar
          </button>
          ...
          <button onClick={() => setTab("orders")} style={{
            ...(tab === "orders" ? premiumButton : ghostButton),
            textAlign:"left"
          }}>
          ...
          <button onClick={() => setTab("chat")} style={{
            ...(tab === "chat" ? premiumButton : ghostButton),
            textAlign:"left"
          }}>
            💬 Müştəri Chat
          </button>

          <button onClick={() => setTab("vacancy")} style={{
            ...(tab === "vacancy" ? premiumButton : ghostButton),
            textAlign:"left"
          }}>
...
textShadow: "0 0 12px rgba(212,175,55,0.35)" }}>{p.price} AZN</p>
      ...
      {tab === "orders" && (
        <div style={{
          ...
          <h2 style={{color:"#D4AF37"}}>Sifarişlər</h2>

          <button onClick={loadOrders} style={premiumButton}>
            Yenilə 🔄
          </button>

          <div style={{display:"grid", gap:14, marginTop:20}}>
            {orders.length === 0 && <p>Hələ sifariş yoxdur.</p>}
            ...
            {orders.map(o => (
              <div key={o.id} style={{
                ...
                <h3 style={{marginTop:0}}>📦 {o.product_name}</h3>
                <p><b>Ad:</b> {o.customer_name}</p>
                <p><b>Telefon:</b> {o.phone}</p>
                <p><b>Qeyd:</b> {o.note || "Yoxdur"}</p>
                <p style={{opacity:.7}}>{o.created_at}</p>

                <button onClick={() => completeOrder(o.id)} style={premiumButton}>
                  Tamamlandı ✅
                </button>
              </div>
            ))}
...
backdropFilter: "blur(10px)"
          ...
          <h2 style={{color:"#D4AF37"}}>Vakansiya müraciətləri</h2>

          <button onClick={loadVacancies} style={premiumButton}>
            Yenilə 🔄
          </button>

          <div style={{display:"grid", gap:14, marginTop:20}}>
            {vacancies.length === 0 && <p>Hələ vakansiya müraciəti yoxdur.</p>}
            ...
            {vacancies.map(v => (
              <div key={v.id} style={{
                ...
                <h3 style={{marginTop:0}}>👷 {v.fullname}</h3>
                <p><b>Telefon:</b> {v.phone}</p>
                <p><b>Vəzifə:</b> {v.position}</p>
                <p><b>Təcrübə:</b> {v.experience || "Yoxdur"}</p>
                <p><b>Qeyd:</b> {v.note || "Yoxdur"}</p>
                <p style={{opacity:.7}}>{v.created_at}</p>

                <button onClick={() => deleteVacancy(v.id)} style={premiumButton}>
                  Baxıldı ✅
                </button>
              </div>
            ))}
