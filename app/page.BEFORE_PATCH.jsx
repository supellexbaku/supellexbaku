...
export default function Page() {
  ...
  useEffect(() => {
    fetch(`${API}/categories`)
      ...
      .catch(() => {});
    const savedFavs = localStorage.getItem("supellex_favourites");
    if (savedFavs) {
      try {
  ...
  function toggleFavourite(product) {
    const exists = favourites.some((x) => x.id === product.id);

    const next = exists
      ? favourites.filter((x) => x.id !== product.id)
      : [...favourites, product];

    setFavourites(next);
    localStorage.setItem("supellex_favourites", JSON.stringify(next));
  }

  function isFavourite(product) {
    return favourites.some((x) => x.id === product.id);
  }
              ...
              <div style={{
                ...
                <button
                  ...
                  style={{
                    float:"right",
                    border:"1px solid rgba(255,255,255,0.10)",
                    background:isFavourite(p) ? "#D4AF37" : "rgba(255,255,255,0.06)",
                    color:isFavourite(p) ? "#000" : "#fff",
                    borderRadius:16,
                    padding:"8px 12px",
                    fontSize:18
                  }}
