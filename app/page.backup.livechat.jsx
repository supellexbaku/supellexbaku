...
export default function HomePage() {
  ...
  async function sendSupportMessage() {
    if (!supportMessage.trim()) {
      setSupportStatus("Mesaj boş ola bilməz.");
      return;
    }

    const response = await fetch(
      "https://friendly-palm-tree-4q7jrr4757v4h5j45-3000.app.github.dev/support/message",
      {
        method: "POST",
        headers: {
    ...
    setSupportStatus(data.reply || "Mesaj göndərildi ✅");
  ...
  async function checkSupportReply() {
    ...
    const response = await fetch(
      `https://friendly-palm-tree-4q7jrr4757v4h5j45-3000.app.github.dev/support/message/${supportRequestId}`
    );

    const data = await response.json();

    if (data.ok && data.message && data.message.reply) {
      setMessages((prev) => [
        ...prev,
        {
          role: "bot",
          text: "Canlı dəstək cavabı: " + data.message.reply,
        },
      ]);

      setSupportStatus("Canlı dəstək cavab verdi ✅");
  ...
  return (
    <>
      ...
      <main style={{ minHeight: "100vh", background: "var(--black)", color: "var(--text)", overflowX: "hidden" }}>
        ...
        >
          <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: "0.08em" }}>AI</span>
        </button>
          ...
          <div style={{
            ...
            <div style={{
              ...
              {showLiveSupport && (
                ...
                >
                  <h3 style={{ color: "var(--gold)", fontSize: 14 }}>
                    Canlı dəstək
                  </h3>
                  ...
                  <button onClick={sendSupportMessage} className="btn-gold">
                    Canlı dəstəyə göndər
                  </button>

                  {supportRequestId && (
                    <button
                      onClick={checkSupportReply}
                      className="btn-outline"
