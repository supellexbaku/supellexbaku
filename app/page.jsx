...
function AiPremiumIcon() {
  return (
    <svg viewBox="0 0 80 80" className="w-10 h-10">
      <defs>
        <linearGradient id="aiPremium" x1="0" x2="1" y1="0" y2="1">
      ...
      <circle cx="40" cy="40" r="34" fill="url(#aiPremium)" opacity="0.95" />
...
export default function Page() {
  ...
  const formItems = [
    ["Geri bildiriş / Şikayət", "Narazılıq, təklif və problem yazın"],
    ["Vakansiya", "Usta kimi işləmək istəyənlər üçün"],
    ["Reklam / Tərəfdaşlıq", "Əməkdaşlıq müraciəti göndərin"],
    ["İkinci əl mebel al/sat", "Almaq və ya satmaq istəyənlər üçün"],
  ];
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white overflow-hidden">
      ...
      <section className="min-h-screen flex items-center justify-center px-5 pt-24 bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900">
        <div className="max-w-5xl text-center">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <a href="https://wa.me/994554131658" target="_blank" className="px-8 py-4 rounded-2xl bg-green-500 font-bold inline-flex items-center gap-3">
              <WhatsAppIcon /> WhatsApp
            </a>

            <a href="https://t.me/supellex_baku_bot" target="_blank" className="px-8 py-4 rounded-2xl bg-sky-500 font-bold inline-flex items-center gap-3">
            ...
            {products.map((item) => (
              <div key={item} className="rounded-3xl bg-slate-100 overflow-hidden shadow-xl">
                ...
                <div className="p-6">
                  ...
                  <a href="https://wa.me/994554131658" target="_blank" className="inline-flex items-center gap-2 mt-5 bg-green-500 text-white px-5 py-3 rounded-xl font-bold">
      ...
      <section id="vakansiya" className="max-w-7xl mx-auto px-5 py-24 grid md:grid-cols-2 gap-6">
        {formItems.map(([title, desc]) => (
          <form
            key={title}
            onSubmit={(e) => {
              e.preventDefault();
            ...
            className="rounded-3xl bg-white/10 border border-white/10 p-6 shadow-2xl"
          >
            <h3 className="text-2xl font-black text-blue-300">{title}</h3>
            <p className="text-white/60 mt-2">{desc}</p>

            <input
              className="w-full mt-5 p-4 rounded-xl bg-white text-slate-900 placeholder-slate-500 outline-none border border-white/20 focus:border-blue-400"
              placeholder="Adınız"
            />

            <input
              className="w-full mt-3 p-4 rounded-xl bg-white text-slate-900 placeholder-slate-500 outline-none border border-white/20 focus:border-blue-400"
              placeholder="Telefon nömrəsi"
            />

            <textarea
              className="w-full mt-3 p-4 rounded-xl bg-white text-slate-900 placeholder-slate-500 outline-none border border-white/20 focus:border-blue-400 h-28"
              placeholder="Mesajınız"
            ></textarea>

            <button className="mt-4 w-full bg-blue-600 hover:bg-blue-700 py-4 rounded-xl font-bold">
              Göndər
            </button>
          </form>
        ))}
      ...
      <section id="rey" className="px-5 py-24 bg-white text-slate-900">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <input className="w-full p-4 rounded-xl bg-white text-slate-900 placeholder-slate-500" placeholder="Adınız" />

            <select className="w-full p-4 rounded-xl mt-3 bg-white text-slate-900">
            ...
            <textarea className="w-full p-4 rounded-xl mt-3 h-28 bg-white text-slate-900 placeholder-slate-500" placeholder="Rəyiniz"></textarea>
        ...
        <div className="fixed inset-0 z-[998] bg-slate-950 text-white p-5 overflow-auto">
          ...
          <div className="max-w-4xl mx-auto pt-20">
            ...
            <div className="mt-8 bg-white text-slate-900 rounded-3xl p-6">
              ...
              <textarea className="w-full mt-5 border p-4 rounded-2xl h-32 bg-white text-slate-900 placeholder-slate-500" placeholder="Mesajınızı yazın..."></textarea>
